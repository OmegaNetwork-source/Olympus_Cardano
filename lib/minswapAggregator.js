...
/**
 * CIP-30: signTx returns a witness set hex. Minswap finalize attaches it and submits.
 * Fallback: Mesh BrowserWallet signTx + submitTx (full signed CBOR).
 *
 * @param {{
 *   walletKey: string,
 *   api?: object,
 *   unsignedCbor: string,
 * }} opts
 * @returns {Promise<{ txId: string, via: "minswap" | "wallet" }>}
 */
export async function minswapSignAndSubmit(opts) {
  ...
  if (cip30?.signTx) {
    try {
      // partialSign=true is safer for aggregator txs that may need extra witnesses.
      ...
      // Fall through to Mesh path (some wallets return full tx / reject partial).
      console.warn("[minswap] CIP-30 finalize path failed, trying wallet submit", e?.message || e);
    }
  }

  const { BrowserWallet } = await import("@meshsdk/core");
  const wallet = await BrowserWallet.enable(opts.walletKey);
  const signed = await wallet.signTx(unsignedCbor);
  const txId = await wallet.submitTx(signed);
  return { txId: String(txId), via: "wallet" };
}
